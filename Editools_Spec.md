...
- **Mesh persistence** (`GreyPrimitive.PrepareBakeTargets` / `PersistBakedMeshes`, all push-based at bake time — nothing rebuilds on load or domain reload):
  - Scene-owned (plain scene object, prefab instance in a scene, unpacked instance) → meshes serialized scene-embedded; `_meshOwnerId` (instance id) makes a duplicated object split off its own mesh on its next rebake
  - Prefab-owned (prefab stage, or the asset itself) → meshes embedded inside the prefab file itself as sub-assets (render mesh + collider twin) — no external mesh files; `_meshOwnerGuid` (prefab GUID) marks ownership; instances reference the embedded bake through the prefab
  - **Rebaking a prefab instance never touches the prefab**: the bake forks off the prefab's embedded mesh to a fresh scene-embedded one, and the changed references (`_mesh`, `MeshFilter`/`MeshCollider`) serialize as regular prefab instance overrides — revert restores the prefab's bake, other instances are unaffected. The same fork rule holds mid-drag (`EnsureLiveMeshTargets`), so a drag on an instance can't deform the shared bake in memory
  - **Applying instance overrides re-bakes the prefab** (`GreyPrimitiveApplyGuard`, hooks `PrefabUtility.prefabInstanceUpdated`): an applied local bake would leave the prefab pointing at a scene mesh, so the guard detects the invalid state (`PrefabAssetBakeIsValid`), re-bakes the asset component in place with the applied parameters, and sweeps grey bake meshes orphaned in the file (name-gated to `Grey*` and unreferenced under the root — foreign mesh sub-assets are never touched)
  - Duplicates split off automatically: a duplicated object inside the same prefab root is detected by reference-sharing scan, a duplicated prefab file by GUID mismatch; a superseded mesh embedded in the same prefab is removed on split so sub-assets don't accumulate
